...
func TestSpecValidate(t *testing.T) {
	tests := []struct {
		...
		{"unknown type", &Spec{Type: "azure"}, false, `must be local, gcs or s3`},
		{"s3", &Spec{Type: BackendS3, Bucket: "b", Prefix: "kb/memory/", Endpoint: "https://s3.example.net", Region: "garage", PathStyle: true}, false, ""},
		{"s3 on AWS with SSE", &Spec{Type: BackendS3, Bucket: "b", Prefix: "kb/memory/", SSE: "AES256"}, false, ""},
		{"s3 with no bucket", &Spec{Type: BackendS3, Prefix: "p/"}, false, "bucket is required"},
		{"s3 with an s3:// bucket", &Spec{Type: BackendS3, Bucket: "s3://b/x", Prefix: "p/"}, false, "must be a bucket name"},
		{"s3 with no prefix", &Spec{Type: BackendS3, Bucket: "b"}, false, "prefix is required"},
		{"s3 with a path", &Spec{Type: BackendS3, Bucket: "b", Prefix: "p/", Path: "x"}, false, "path applies to type: local"},
		{"s3 with a bare endpoint", &Spec{Type: BackendS3, Bucket: "b", Prefix: "p/", Endpoint: "s3.example.net"}, false, "endpoint must be an http(s):// URL"},
		{"s3 with an unknown sse", &Spec{Type: BackendS3, Bucket: "b", Prefix: "p/", SSE: "rot13"}, false, "sse must be AES256 or aws:kms"},
		{"gcs with s3 addressing", &Spec{Type: BackendGCS, Bucket: "b", Prefix: "p/", Endpoint: "https://x"}, false, "endpoint/region/path_style/sse/single_writer apply to type: s3"},
		{"local with s3 addressing", &Spec{Type: BackendLocal, PathStyle: true}, false, "endpoint/region/path_style/sse/single_writer apply to type: s3"},
		{"s3 single writer", &Spec{Type: BackendS3, Bucket: "b", Prefix: "p/", SingleWriter: true}, false, ""},
