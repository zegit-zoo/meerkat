...
|---|---|---|---|---|
| addressing | virtual-host (default) | path-style (`path_style: true`) | path-style (`path_style: true`) | n/a |
| `region:` | from the default chain or `region:` | must match `s3_region` (default `garage`) | any (observed with `us-east-1`) | n/a |
| ETag on `HeadObject` / listing | yes | yes | **yes** (observed) | generation |
...
The Garage "observed" cells were measured against `dxflrs/garage:v2.4.1`
on 2026-09-18; the Garage write gap is expected to close in a future
...
provider's documented behaviour; the AWS row is asserted by the same
tests when the matrix runs against it
(`MEERKAT_TEST_S3_PROVIDER=aws`).

The Versity Gateway cells marked "observed" were measured on
2026-09-25 against
`ghcr.io/versity/versitygw:v1.8.0@sha256:30292fc2eeacc67a36993b01f7a7a5e3361a19cced0e80c1d71cfa2a4b0a2499`
on the posix backend, by the CI job this change added:

```text
provider="versitygw"  PUT If-None-Match:* enforced=true  PUT If-Match enforced=true  GET If-Match enforced=true
```

Two Versity Gateway cells say "not verified": nothing in meerkat
depends on lifecycle rules (see "Retention is an application job"
below) or on SSE against this provider, and no test exercises either —
do not read them as a "yes".
